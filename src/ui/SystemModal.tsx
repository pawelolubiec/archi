  ...
  return (
    <div className="mt-5 space-y-3">
      {cards.map((card) => (
        ...
        >
          <div className="text-sm font-semibold text-paper">{card.claim}</div>
          <p className="mt-1.5 text-xs leading-relaxed text-mist">{card.support}</p>
          <span className="mt-2 inline-block rounded-md border border-gold/25 bg-gold/10 px-2 py-0.5 font-mono text-[10px] uppercase tracking-[0.14em] text-gold/90">
            {card.tag}
          </span>
        </div>
      ))}
